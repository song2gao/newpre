...
public class DataCenterThread extends Thread {
    ...
    public void run() {
        ...
        while (true) {
            try {
                List<PomsEnergyUsingSystem> systemList = BussinessConfig.systemList;
                PomsEnergyUnitInfo unit = new PomsEnergyUnitInfo();
                unit.setEuiCode("1");
                unit.setSystems(systemList);
                HttpRequester requester = new HttpRequester();
                Map<String, String> parameters = new HashMap<String, String>();
                String deviceDataJson = JSON.toJSONString(unit);
                ...
                for (String url : urls) {
                    ...
                    if (respons.getCode() == 200) {
                        logger.info("<==============数据上传服务器" + url + "成功================" + dateTime + ">");
                    } else {
                        logger.info("<==============数据上传服务器" + url + "失败" + respons.getContent() + "================" + dateTime + ">");
