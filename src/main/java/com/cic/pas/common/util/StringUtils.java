...
public class StringUtils {
    ...
    public static void get645SendStr(String ctdCode,int mdAddress, BigDecimal pdAddress,Map<String,byte[]> map) {
    ...
    public static String toSubtract645(String str) {
        ...
        for (int i = 0; i <= str.length() - 2; i += 2) {
            String subStr = str.substring(i, i + 2);
            String res = Integer.toHexString(Integer.parseInt(subStr, 16) - Integer.parseInt("33", 16));
            res=StringUtils.leftPad(res, 2, '0');
            res = res.substring(res.length() - 2);
            result += res;
        }
        return result;
