...
public class CRC16M {
    ...
    public static byte[] subData(byte[] bytes, int begin, int length) {
    ...
    public static byte[] getModTcp(String toSend) {
        return hexStringToByte("000000000006" + toSend);
    }
     ...
     * @return
     */
    public static String getBufHexStr(byte[] raw, int begin, int end) {
        if (raw == null) {
            return null;
        }
        final StringBuilder hex = new StringBuilder(2 * (end - begin));
    ...
    public static String getHexStr(byte[] raw) {
        ...
        for (final byte b : raw) {
            ...
            String hex = StringUtils.leftPad(Integer.toHexString(b & 0xff), 2, '0');
            sb.append(hex);
            i++;
        }
        sb.append("]");
     ...
     * @return 完整协议byte[]
     */
    public static byte[] getSendBuf(byte[] bytes) {
        int CRC = 0x0000ffff;
        ...
        result[bytes.length] = high;
        result[bytes.length + 1] = low;
        return result;
    }
}
