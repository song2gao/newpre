...
public class HttpRequestDecoder extends MessageDecoderAdapter {
	...
	private Map<String, String[]> parseRequest(StringReader is) {
		...
		try {
			...
			map.put("Type", new String[] { url[0].toUpperCase() });
			map.put("Method", new String[] { url[1].substring(1) });
			map.put("Protocol", new String[] { url[2] });
			// Read header
			while ((line = rdr.readLine()) != null && line.length() > 0) {
				String[] tokens = line.split(":");
				map.put(tokens[0].replaceAll(" ", ""),
						new String[] { tokens[1] });
			}

			// If method 'POST' then read Content-Length worth of data 5
			if (url[0].equalsIgnoreCase("POST")) {
				String[] values = map.get("Content-Length");
